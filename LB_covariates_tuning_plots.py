...
for i in range(num_tuning):
    file = "Results/LB_covariates_res/setting_1/LinReBootG_res_" + str(i) + ".csv"
...
for i in range(num_tuning):
    file = "Results/LB_covariates_res/setting_2/LinReBootG_res_" + str(i) + ".csv"
    label = "LinReBoot-G(" + str(weight_sd_list[i]) + ")"
    res = pd.read_csv(file)
...
for i in range(num_tuning):
    file = "Results/LB_covariates_res/setting_3/LinReBootG_res_" + str(i) + ".csv"
    label = "LinReBoot-G(" + str(weight_sd_list[i]) + ")"
    res = pd.read_csv(file)
...
#plt.show()
fig.savefig('Results/LB_covariates_res/LB_covariates_tuning_plot_summary.png', dpi=250)